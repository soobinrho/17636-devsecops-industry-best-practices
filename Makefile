...
create-prod-ssh-key:
	if [ ! -f ./server-prod/17636-prod-ssh.key ]; then \
		echo '[INFO] This Makefile target is not meant to be automated. Instead, I choose to add the ...key value to Jenkins credentials manually for security purposes.'; \
		yes | ssh-keygen -q -N "" -f ./server-prod/17636-prod-ssh.key; \
	fi
